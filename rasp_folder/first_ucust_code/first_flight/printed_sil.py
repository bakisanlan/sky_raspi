...
if home_location == None:
    home_location = LocationGlobal(40.2320412, 29.0083218,100)
        ...
        if vehicle.mode == VehicleMode("AUTO") and (not isFront_bombed):
            ...
            if detected_obj_px != None:
                ...
                if True:
                    ...
                    ned_guess = geodetic_to_NED(vehicle.location.global_relative_frame,LocationGlobalRelative(obj_mean_lat_lon[0],obj_mean_lat_lon[1],0))                   

                    print_now()
                    print('Objenin ilk tahmini koordinatlari')
                    print('lat: {}, lon {}'.format(obj_mean_lat_lon[0],obj_mean_lat_lon[1])) 
                    print('NED of object rel to vehicle: {}'.format(ned_guess))
                    while True: #4
                        camera_bot = camera_class(video,fourcc,out)
                        detected_obj_px = camera_bot.detect_x_y(frame_size,False,True) 
                                ...
                                if detected_obj_px != None:
                                    ...
                                    while (time.time() - time_start) < 2:  #!
                                        ...
                                        if detected_obj_px != None:   
                                            ...
                                            ned_guess = geodetic_to_NED(vehicle.location.global_relative_frame,LocationGlobalRelative(final_obj_mean_lat_lon[0],final_obj_mean_lat_lon[1],0))

                                            print_now()
                                            print('Objenin ikinci tahmini koordinatlari')
                                            print('lat: {}, lon {}'.format(final_obj_mean_lat_lon[0],final_obj_mean_lat_lon[1])) 
                                            print('NED of object rel to vehicle: {}'.format(ned_guess))
                                            ...
                                            while True: #2
                                                ...
                                                if True:
                                                    ...
                                                    while True: #1
                                                        ...
                                                        if True:
                                                        #if (dist_from_drop_point < back_servo_time*vehicle.groundspeed) and (abs(bearing_to_wp - math.degrees(vehicle.attitude.yaw)) < 15):
